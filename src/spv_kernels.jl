...
# Lexicographic (x, z) key order, x-major. Works on 2-tuples and on the
# (z, x, c) workspace triples (fields stay laid out as (z, x[, c]); only the
# comparison order is x-first). x-major means all terms sharing an x-string
# are CONTIGUOUS in the live buffer. Since a Pauli (z, x) maps a basis ket
# |v⟩ to ±i^k|v ⊻ x⟩, an x-run is exactly the set of terms sharing a target
# ket — the invariant the fused truncation corrections (truncation.jl), the
# ordered-pass variance, and the diagonal-prefix expectation value rely on.
# The identity key (0, 0) remains the minimum (tr() reads slot 1), and the
# diagonal (x = 0) block is the prefix of the live buffer.
...
iterate-larger) of tuples by their first two fields — the x-major key for
...
# key is (x, z) lexicographic: x is the high word. The "high prefix" above
# bit b of the x word ignores z entirely; above bit b of the z word it
# includes all of x. Prefix tuples are (x-part, z-part) so tuple comparison
# matches the x-major key order.
@inline _uw(t::Tuple{W,W,T}, ::Val{1}) where {W,T} = t[1]
@inline _uw(t::Tuple{W,W,T}, ::Val{2}) where {W,T} = t[2]
@inline _uhp(t::Tuple{W,W,T}, hb::W, ::Val{1}) where {W,T} = (t[2], t[1] & hb)
@inline _uhp(t::Tuple{W,W,T}, hb::W, ::Val{2}) where {W,T} = (t[2] & hb, zero(W))
...
Sort `v.ws[1:m]` by the x-major key, exploiting rotation structure: sin-branch
...
function _unshuffle_ws!(v::SparsePauliVector{N,W,T}, m::Int, gz::W, gx::W) where {N,W,T}
    ...
    src, dst = v.ws, v.ws2
    flips = 0
    # Highest combined key bit first: under x-major order every x bit
    # outranks every z bit, so the x-word passes run before the z-word ones.
...
# and retry.
# `sink` observes each net-merged coefficient the filter rejects, and (for
# the run sinks) each survivor — the emission order is x-major sorted, the
# invariant XRunDelta relies on (see truncation.jl); `nothing` compiles both
# hooks away. The sink is reset at the start of every attempt so a
# grow-and-retry after overflow doesn't double-count.
function _try_merge!(v::SparsePauliVector{N,W,T}, m::Int, f::MergeFilter,
                     sink::SPVSink=nothing) where {N,W,T}
    _sink_reset!(sink)
    n = v.n
    z, x, c = v.z, v.x, v.c
...
# `sink` observes dropped terms and (run sinks) survivors, in the x-major
# order the live buffer is stored in (nothing = today's kernel).
function _compact_spv!(v::SparsePauliVector{N,W,T}, f::MergeFilter,
    ...
    @inbounds for i in 1:v.n
        ...
        _sink_keep!(sink, v.z[i], v.x[i], v.c[i])
        out += 1
        if out != i
            v.z[out] = v.z[i]
            v.x[out] = v.x[i]
...
# contributes c · (-1)^popcount(z & ψ) iff x == 0. Under x-major order the
# diagonal (x = 0) terms are a contiguous PREFIX of the live buffer, so the
# live scan stops at the first off-diagonal term; the append buffer is
# unsorted and still scanned in full.
