...
ShardedCounters(nw::Int) = ShardedCounters(zeros(Int, nw), zeros(Int, nw),
...
function merge_shards!(S::ShardedPauliSum{N,W,T}, f::MergeFilter;
                       counters::Union{Nothing,ShardedCounters}=nothing,
                       w::Int=1) where {N,W,T}
    maxpop = 0
...
function inner_product(S1::ShardedPauliSum{N,W,T}, S2::ShardedPauliSum{N,W,T}) where {N,W,T}
    ...
    for j in 1:nshards(S1)
        ...
        @inbounds while i <= a.n && k <= b.n
            ka = (a.z[i], a.x[i])
            kb = (b.z[k], b.x[k])
            if _shard_key_eq(ka, kb)
                out += conj(a.c[i]) * b.c[k]
                i += 1
                k += 1
            elseif _shard_key_lt(ka, kb)
                i += 1
            else
                k += 1
            end
...
function greedy_bisection_rankmap(S::ShardedPauliSum{N,W,T}, r::Int; kw...) where {N,W,T}
    terms = Vector{PauliBasis{N,W}}(undef, length(S))
    i = 0
    for sh in S.shards
        for k in 1:sh.n
            i += 1
