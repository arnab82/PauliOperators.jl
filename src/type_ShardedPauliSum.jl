...
# `_word_type`, `_pack`, `_unpack`, `_HIST_BINS` and `_engine_coeff` are the
# bitint versions from word.jl / type_SparsePauliVector.jl -- the shared1 copies
# were removed so the sharded engine inherits BitIntegers storage (N > 128)
# instead of capping at UInt128.
...
# _engine_coeff (both the Real and Complex methods) comes from bitint's
# type_SparsePauliVector.jl -- identical definitions, kept in one place.
...
function Base.get(S::ShardedPauliSum{N,W,T}, p::PauliBasis{N}, default) where {N,W,T}
    ...
    @inbounds while lo <= hi
        ...
        if sh.z[mid] == z && sh.x[mid] == x
            return sh.c[mid]
        elseif _shard_key_lt((sh.z[mid], sh.x[mid]), (z, x))
            lo = mid + 1
        else
            hi = mid - 1
        end
...
function check_sharding(S::ShardedPauliSum{N,W,T}) where {N,W,T}
    for (j, sh) in enumerate(S.shards)
        ...
        for i in 1:sh.n
            ...
            if i > 1
                _shard_key_lt((sh.z[i-1], sh.x[i-1]), (sh.z[i], sh.x[i])) ||
